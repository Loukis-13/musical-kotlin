...
class EstiloServiceImpl(val estiloRespository: MusicalRepository<Estilo>) : MusicalService<Estilo> {
    ...
    override fun searchEntity(id: Int) = ResponseEntity.ok(estiloRespository.findById(id).orElseThrow())
    ...
    override fun deleteEntity(id: Int): ResponseEntity<Void> = estiloRespository.deleteById(id).run {
